...
class PPOTrainer:
    ...
    def step(self, query, response, scores, seq2seq: bool = False):
        """
        ...
        #TODO: cat query and response
        # model_input = torch.cat((query, response), axis=1)
        model_input = query

        t = time.time()
        if seq2seq is True:
            logprobs, ref_logprobs, values = self.batched_forward_pass(model_input, gen_len, decode_inputs=response)
        else:
    ...
    def batched_forward_pass(self, model_input, gen_len, decode_inputs=None):
        ...
        for i in range(int(bs/fbs)):
            ...
            if decode_inputs is not None:
                ...
                outputs = self.model(input_ids=m_input, decoder_input_ids=m_decode_ids)
                logits, _, v = outputs
