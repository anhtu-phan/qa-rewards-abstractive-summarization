...
if model_type == "google_pegasus_xsum":
    summary_model = PegasusForConditionalGeneration.from_pretrained(pretrained_model_name_or_path=model_path).to(device)
    summary_model_ref = PegasusForConditionalGeneration.from_pretrained(pretrained_model_name_or_path=model_ref_path).to(device)
    ...
    summary_model = GPT2Model.from_pretrained(pretrained_model_name_or_path=model_path, max_length=max_token_len).to(device)
    summary_model_ref = GPT2Model.from_pretrained(pretrained_model_name_or_path=model_ref_path, max_length=max_token_len).to(device)
else:
...
def main():
    ...
    for i in tqdm(range(0, df.shape[0], batch_size)):
        ...
        response_ids = summary_model.generate(encoding['input_ids'].to(device))
        response_ids_ref = summary_model_ref.generate(encoding['input_ids'].to(device))

        response = summary_tokenizer.batch_decode(response_ids, skip_special_tokens=True)
        response_ref = summary_tokenizer.batch_decode(response_ids_ref, skip_special_tokens=True)
