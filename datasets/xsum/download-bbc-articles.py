...
def DownloadMode(urls_file, missing_urls_file, downloads_dir, request_parallelism, timestamp_exactness):
    ...
    print(f'Downloading URLs from the {urls_file}file:')
    ...
    if os.path.exists(missing_urls_filename):
        ...
        urls_valid_todownload = list(
            set(urls_full).intersection(ReadUrls(missing_urls_filename)))
    ...
    while(len(urls_left_todownload) != 0):
        ...
        results = p.imap_unordered(DownloadMapper, zip(
            urls_left_todownload, repeat(downloads_dir), repeat(timestamp_exactness)))
        ...
        print(f'Reset urls_left_todownload - (collected_urls:{len(collected_urls)}, missing_urls:{len(missing_urls)})')
        urls_toignore = collected_urls[:] +  missing_urls[:]
        urls_left_todownload = list(
            set(urls_left_todownload) - set(urls_toignore))
        print('urls left to download: ' + str(len(urls_left_todownload)))
...
def main():
    ...
    urls_file_to_download = "/Users/phananhtu/Projects/Heidelberg/qa-rewards-abstractive-summarization/datasets/xsum/XSum-WebArxiveUrls.txt"
    ...
    DownloadMode(urls_file_to_download, missing_urls_file, downloads_dir,
                 args.request_parallelism, args.timestamp_exactness)
